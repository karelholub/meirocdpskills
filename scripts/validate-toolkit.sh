...
ruby -rjson -ryaml -e '
  ...
  abort "expected 16 skills" unless files.length==16
  files.each do |file|
...
if grep -R -n -E '\[TODO:|/Users/[^/]+/|BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY|Bearer [A-Za-z0-9._-]{16,}' "$ROOT" --exclude-dir=.git --exclude=validate-toolkit.sh; then
